...
- 新增 Go BitTorrent sidecar：支持磁力链接与 `.torrent` URL、DHT / PEX / uTP / TCP 发现，连接数 8–500，并以独立进程隔离复杂协议栈。
- 新增**纯 RAM 分片存储**：分片校验后立刻释放，只保留完成位图；禁止上传与 web seed，不会将 torrent 内容写入磁盘。
- 新增连接保活与死链分析：展示握手、活跃 / 待连接 / 半开连接、做出有效贡献 / 卡住 / 已关闭 / 死链节点、tracker 成败及无效流量。
- 新增 PT 侧 RAM、水位、坏分片、存储错误和压力状态；结合 Windows 网卡利用率、错误与丢弃，给出「本机网卡、远端 swarm、或链路」的证据式瓶颈提示，不把路由器归因伪装成确定结论。
- 明确保留原有「局域网 HTTP 分片仿真」作为内网稳定回放；真实公网性能测试改由新 PT 路径承担。
- Go sidecar 作为桌面应用内嵌资源：安装版与单文件 portable 版首次运行会校验并释放到用户临时目录，无需手工携带额外 EXE。

### 验证与发布

- CI 新增 Go 单元测试、`go vet` 与 Windows sidecar 构建；发布构建预先生成内嵌 sidecar。
- 使用 Ubuntu 官方 torrent 进行了公网 smoke test：真实发现和连接 peer，分片校验后 RAM 占用回收，无持久化数据文件。
